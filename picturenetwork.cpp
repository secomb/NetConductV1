...
void picturenetwork(float *nodvar, float *segvar, const char fname[])
{
	...
	picfac = FMIN(450. / xmax, 700. / ymax);//updated April 2010
	ofp = fopen(fname, "w");
	...
	xzmin = -70;
	//for (iseg = 1; iseg <= nseg; iseg++) if (segtyp[iseg] == 4 || segtyp[iseg] == 5) {
	for (iseg = 1; iseg <= nseg; iseg++) if (cnode[3][ista[iseg]] < 100.) {		//just top of network
			fprintf(ofp, "%g sl\n", picfac*diam[iseg]);
...
//label nodes
	...
	for(inod=1; inod<=nnod; inod++) if (cnode[3][inod] < 100.) { //just top of network
		xs = 0.;
		ys = 0.;
		for (i = 1; i <= 3; i++) {
		...
		fprintf(ofp, "%g mx %g my m ", xs + 3*picfac, ys);
		fprintf(ofp, "(%g) show\n", nodvar[inod]);
	}
//label segments
